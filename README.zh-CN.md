...
- **返回顶部与随机漫游**：一键回到当前文档顶部，或随机打开一篇笔记，漫游你的知识库。配合前进/后退，可以完整回到之前浏览过的文档。
...
- **一键分享与导入**：支持将单个动作按钮（含自定义 JS 脚本）或整套预设一键导出为分享超链接。点击链接即可在电脑或手机上直接导入，或者在设置页粘贴分享码导入，方便多端配置同步与备份。
- **思源原生风格模式**：一键把移动端导航栏切换为贴近思源自带底部导航栏的视觉（随主题的胶囊外观），想与思源原生融为一体时用这个风格。
- **隐藏原生底部导航栏**：可隐藏思源自带的移动端底部导航栏，让悬浮导航栏完全接管底部区域，想恢复随时可在设置里关掉。
- **从导航栏打开思源设置**：内置一个「打开思源设置」动作，移动端打开与原生底栏「更多」一致的菜单，桌面端打开思源设置面板，与插件自身的设置入口区分开。
...
- **按钮标签文字**：随时控制按钮下方是否显示文字说明。位于「外观」页、风格模式下方；选择「思源原生」风格时自动禁用。
- **风格模式**：切换移动端导航栏为插件的灵动岛风格，或与思源原生底栏融合的思源原生风格。
- **隐藏原生底部导航栏**：隐藏思源自带的移动端底部导航栏，让悬浮导航栏完全接管；关闭开关即可恢复。
...
| 类型                    | 说明                                                                                                                                                                                                                                                                                                                                                                                        |
| ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **内置功能 (Builtins)** | 插件自带的加强版功能，包括：<br/>• **文档层级漫游**：快速跳转父级、子级、相邻文档、后退/前进<br/>• **返回顶部与随机文档**：一键回到顶部、滚动到底部，或随机打开一篇笔记<br/>• **上传并插入图片**：一键从相册/文件管理器中选择图片上传。支持在光标处插入；若处于只读模式或未点击聚焦文档时，会自动智能追加至文档末尾，方便快捷操作<br/>• **自定义链接/文档ID**：打开外部网页或思源某篇文档，完美兼容移动端<br/>• **随机漫游 (SQL)**：用自定义 SQL 从笔记海里随机翻阅<br/>• **添加到数据库**：一键把当前文章添加进指定的属性视图里<br/>• **执行自定义 JS 脚本**：在思源环境中直接运行代码，内置封装了常用的思源工具箱，方便快速实现复杂逻辑 |
