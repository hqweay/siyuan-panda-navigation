...
## ⚠️ 核心设计模式：编辑器状态分流写入 (Document Writing Pattern)

向当前文档写入内容时，必须遵循以下第一性原理模式：
1. **编辑器获取**：使用 `siyuan.getActiveEditor(false)` 获取当前活动的编辑器。必须传递 `false` 参数，以防因弹出窗口、输入框等导致的主窗口失去焦点，致使获取到 `undefined`。
2. **状态分流写入**：
   - **交互式插入（有选区且可编辑）**：当编辑器存在，且非只读（`!editor.protyle.disabled`），且当前具有活动光标/选区（即选区在编辑器内）时，使用 `editor.insert(编译后的DOM)` 进行插入。*注意：思源原生编辑器不接受原始 Markdown 文本，必须用 Lute 编译：`let html = editor.protyle.lute.Md2BlockDOM(md); html = editor.protyle.lute.BlockDOM2InlineBlockDOM(html); editor.insert(html, false, true);`*
   - **后台/只读追加（无选区或只读）**：若编辑器处于只读模式，或用户未点击聚焦文档，则直接降级为调用后台 REST API，向文档直接追加内容。这样可保证 100% 成功率且防报错：
     `await fetchSyncPost("/api/block/appendBlock", { data: md, dataType: "markdown", parentID: editor?.protyle?.block?.rootID || await kits.getActiveDoc() })`
